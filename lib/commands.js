...
function toggleCitations() {
    ...
    if (!editorHelpers.isAnythingSelected()) {
        editorHelpers.surroundSelection(">", "", /> .+|.+/gi)
        return;
    }

    if (editorHelpers.isMatch(HasCitations)) {
        editorHelpers.replaceBlockSelection((text) => text.replace(HasCitations, "$1$2"))
    }
    else {
        editorHelpers.replaceBlockSelection((text) => text.replace(AddCitations, "$1> $2"))
    }
}
